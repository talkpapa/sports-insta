...
async function main() {
  ...
  /* 원고가 "이 소재는 쓸 수 없다"고 되돌려 보내는 일이 잦다.
   *
   * 설명형으로 바꾼 뒤로는 "무슨 일이 있었다"만 있고 왜인지가 없는 글이 전부
   * 걸러진다 — 8월 24일에는 여덟 건 중 여섯 건이 그렇게 빠져 두 편밖에 못 만들었다.
   * 지어내지 않는 것이 옳으므로 규칙은 그대로 두고, 대신 후보를 넉넉히 받는다.
   * 건너뛴 것도 무료 한도를 한 번씩 쓰지만, 모델 다섯 개로 하루 100건까지 되므로
   * 세 편 만드는 데 열세 번 두드리는 것은 감당된다. */
  const news = await collectNews(target + 10);
